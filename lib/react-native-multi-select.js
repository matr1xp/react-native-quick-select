...
export default class MultiSelect extends Component {
  ...
  _displaySelectedItems = () => {
    ...
    if (items && items.length > 0) {
      items_to_render = selectedItems.map(singleSelectedItem => {
        ...
        let itemState = find(this.state, function (o) { return o.item === displayItem })
        return (
          item ?
          <View
          ...
          style={[
            styles.selectedItem,
            {
              ...
              width: itemState && itemState.width > 0 ? itemState.width + 50 : (displayItem.length * 8 + 55),
  ...
  componentDidUpdate (prevProps, prevState) {
    const { items, selectedItems, displayVal } = this.props
    if (prevProps.selectedItems !== selectedItems) {
      if (selectedItems && selectedItems.length > 0) {
        let arrWidths = []
        selectedItems.forEach(id => {
          let selectedItem = find(items, function (o) { return o.id === id})
          if (selectedItem) {
            this._measureWidth(selectedItem[displayVal], this.props).then(result => {
              arrWidths.push({item: selectedItem[displayVal], width: result })
              this.setState({...arrWidths})
            })
          }
        })
      }
  ...
  async _measureWidth(text, props) {
    try{
